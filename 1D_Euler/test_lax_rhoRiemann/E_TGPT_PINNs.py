...
class GPTs(nn.Module):
    def __init__(self, ini, layers, P, c_initial, Nx_train, Nt_train,gamma,Tf):
        ...
        self.linears = nn.ModuleList([nn.Linear(layers[0], layers[0]) for i in range(3*self.layers[1])]+[nn.Linear(layers[1], 1,bias=False) for i in range(3)])
        self.activation = P
        
        for i in range(3*self.layers[1]):
    ...
    def forward(self, x_data):
        ...
        for i in range(0, self.layers[-2]):
            a_data  = self.linears[3*i](test_data)
            b_data  = self.linears[3*i+1](test_data)
            c_data  = self.linears[3*i+2](test_data)
            u_a = self.activation[i](a_data)
            u_b = self.activation[i](b_data)
            u_c = self.activation[i](c_data)
            u_rho = torch.cat((u_rho, u_a[:,0:1]), 1)
            u_p = torch.cat((u_p, u_b[:,1:2]), 1)
            u_u = torch.cat((u_u, u_c[:,2:]), 1)
    ...
    def loss(self, xt_resid, IC_xt,IC_u,BC_xt, BC_u, f_hat,xt_test,xt_en, xt_RH, xt_RHL):
        ...
        loss = WE_loss_R + 10*(loss_IC+loss_BC) +100*loss_RH+10*loss_con
        return loss
