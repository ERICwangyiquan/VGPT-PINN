...
class P(nn.Module):
    def __init__(self, layers, w1, w2, w3, w4,w5,w6,w7, b1, b2, b3, b4,b5,b6,b7):
        super().__init__()
        self.layers = layers
        ...
        self.linears[4].weight.data = torch.Tensor(w5).clone().to(device)
        self.linears[5].weight.data = torch.Tensor(w6).clone().to(device)
        self.linears[6].weight.data = torch.Tensor(w7).clone().to(device)
        ...
        self.linears[4].bias.data = torch.Tensor(b5).clone().to(device)
        self.linears[5].bias.data = torch.Tensor(b6).clone().to(device)
        self.linears[6].bias.data = torch.Tensor(b7).clone().to(device)
