...
def gpt_train(TGPT_PINN, nu, x_int_train,x_ic_train,rho_ic_train,u_ic_train,v_ic_train,p_ic_train,x_bcL_train, rho_bcL_train,u_bcL_train,v_bcL_train,p_bcL_train,x_bcI_train, sin_bcI_train,cos_bcI_train,x_test,ux_test,x_grid,y_grid,rx,ry,epochs_gpt, lr_gpt, tol_gpt):

    def closure():
        optimizer.zero_grad()
        loss_values=TGPT_PINN.loss(x_int_train,x_ic_train,rho_ic_train,u_ic_train,v_ic_train,p_ic_train,x_bcL_train, rho_bcL_train,u_bcL_train,v_bcL_train,p_bcL_train,x_bcI_train, sin_bcI_train,cos_bcI_train)
    ...
    lr_gpt = 0.0001
    optimizer = torch.optim.LBFGS(TGPT_PINN.parameters(),lr=lr_gpt,max_iter=20)
    for i in range(1, epochs_gpt+1):
        if (loss_values< tol_gpt): 
            ...
            print(f'{round(nu,3)} stopped at epoch: {i} | gpt_loss: {loss_values.item()} (TGPT_PINN Stopping Criteria Met)\n')
            break
        ...
        if (i % 10 == 0) or (i == epochs_gpt):
            ...
            if (i % 50 == 0) or (i == epochs_gpt):
                ...
                print(f"layer1:{TGPT_PINN.linears[0].weight.data} and {TGPT_PINN.linears[0].bias.data} and layer3:{TGPT_PINN.linears[-1].weight.data}")
                if (i % 200 == 0):
                    u = TGPT_PINN(ux_test).detach().cpu().numpy()
