...
class NN(nn.Module):    
    ...
    def lossIMR(self, xt_residual):
        ...
        u_x = u_xt[:,[0]] 
        d = (0.1*(torch.abs(u_x)-u_x)+1).detach()
    ...
    def lossBC(self, BC1, BC2):
        ...
        loss_BC1 = self.loss_function(u_BC1,self.nu*torch.ones(BC1.shape[0],1).to(device))
        loss_BC2 = self.loss_function(u_BC2,torch.zeros(BC1.shape[0],1).to(device))
