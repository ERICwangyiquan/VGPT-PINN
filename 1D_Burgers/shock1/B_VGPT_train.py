...
def gpt_train(VGPT_PINN, nu, xt_resid,f_hat, Exact_y0, xt_test, IC_xt, IC_u, BC1, BC2, epochs_gpt, lr_gpt, tol_gpt):
    ...
    for i in range(1, epochs_gpt+1):
        if (loss_values[0] < tol_gpt): 
            ...
            print(f'{round(nu,3)} stopped at epoch: {i} | gpt_loss: {loss_values[0].item()} ,rMAE: {L1_loss}, rRMSE:{L2_loss}(VGPT_PINN Stopping Criteria Met)\n')
            break
            
        else:
            if (loss_values[0].item()<10*tol_gpt):#1e-2for1
                VGPT_PINN.wi.requires_grad = False
                optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, VGPT_PINN.parameters()), lr=lr_gpt)
