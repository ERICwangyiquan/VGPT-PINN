...
def gpt2_train(VGPT_PINN, nu, VGPT_P1,xt_resid,f_hat, Exact_y0, xt_test, IC_xt, IC_u, BC1, BC2, epochs_gpt, lr_gpt, tol_gpt):
    ...
    loss_values = VGPT_PINN.loss(xt_resid, VGPT_P1)
    ...
    for i in range(1, epochs_gpt+1):
        ...
        #VGPT_PINN.wi.data = torch.clamp(VGPT_PINN.wi.data, min=0.1 ,max=10)
        ...
        if (i % 100 == 0) or (i == epochs_gpt):
            ...
            if (i % 1000 == 0) or (i == epochs_gpt):
                ...
                #print(f"layer1:{VGPT_PINN.linears[0].weight.data} and {VGPT_PINN.linears[0].bias.data} and layer3:{VGPT_PINN.linears[-1].weight.data}")
                if (i % 500 == 0):
                    #Burgers_plot(xt_test,abs(VGPT_PINN.forward(xt_test)-Exact_y0),101,201, title=fr"PINN Solution Error $\mu={nu}$")
                    #Burgers_plot(xt_test,VGPT_PINN.forward(xt_test),101,201, title=fr"TVGPT-PINN Solution $\mu={nu}$")
                    if (i == 1000):
                        lr_gpt = 0.1*lr_gpt
                        optimizer = torch.optim.Adam(VGPT_PINN.parameters(), lr=lr_gpt)
