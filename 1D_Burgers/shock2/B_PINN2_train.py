...
def pinn2_train(PINN, nu,P1, xt_resid, IC_xt, IC_u, BC1, BC2,xt_RHL, xt_RHR,xt_RHt,xt_RHtL,f_hat,Exact_y0, epochs_pinn, lr_pinn, tol, xt_test):
    ...
    loss_values = PINN.loss(xt_resid,xt_test, IC_xt, IC_u, BC1, BC2,xt_RHL, xt_RHR,xt_RHt,xt_RHtL,f_hat,P1)
    ...
    def closure():
        optimizer.zero_grad()
        loss_values=PINN.loss(xt_batch, xt_test, IC_xt, IC_u, BC1, BC2,xt_resid, xt_RHL,xt_RHt,xt_RHtL,f_hat,P1)[0]
    ...
    for i in range(1, epochs_pinn+1):
        ...
        if (i == 15000):
            lr_pinn = 0.1*lr_pinn
            optimizer = torch.optim.Adam(PINN.parameters(), lr=lr_pinn)
         #   optimizer = torch.optim.LBFGS(PINN.parameters(), lr=0.1 ,max_iter = 20)
