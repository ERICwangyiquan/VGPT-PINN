...
def initial_u(nu, x, t=0):
    u = nu[0]*torch.ones(x.shape[0],1)
    condition1=(x<=-1/2).cpu()
    condition2=(x>1/2).cpu()
    u = torch.where(condition1,nu[1]*torch.tensor(1.0), u).to(torch.float32)
    u = torch.where(condition2,torch.tensor(0.0), u).to(torch.float32)
    return u

def exact_u(nu,xt):
    u = nu[0]*torch.ones(xt.shape[0])
    condition2 = torch.logical_or(torch.logical_and(xt[:,1]<=(2/nu[1]),(xt[:,0]-((nu[0]+nu[1])*xt[:,1]-1)/2)<=0),torch.logical_and(xt[:,1]>=(2/nu[1]), (xt[:,0]<=(nu[1]/2)*xt[:,1]+(nu[0]/nu[1])-1/2))).cpu()
    condition0 = torch.logical_or(torch.logical_and(xt[:,1]<=(2/nu[1]),(xt[:,0]-(nu[0]*xt[:,1]+1)/2)>=0),torch.logical_and(xt[:,1]>=(2/nu[1]), (xt[:,0]>=(nu[1]/2)*xt[:,1] +(nu[0]/nu[1])-1/2))).cpu()
    u = torch.where(condition2,nu[1]*torch.tensor(1.0), u).to(torch.float32)
...
def exact_u_err(u_exact,u_apr,xt,nu):
    ...
    eta = torch.where(abs(xt[:,0])<0.02,torch.tensor(0.0),torch.tensor(1.0)).reshape(-1,1)
