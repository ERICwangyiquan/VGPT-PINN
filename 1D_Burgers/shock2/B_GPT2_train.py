...
def gpt2_train(GPT_PINN, nu, GPT_P1,xt_resid,f_hat, Exact_y0, xt_test, IC_xt, IC_u, BC1, BC2, epochs_gpt, lr_gpt, tol_gpt):
    ...
    loss_values = GPT_PINN.loss(xt_resid, GPT_P1)
    ...
    for i in range(1, epochs_gpt+1):
        ...
        if (loss_values[0] < tol_gpt): 
            ...
            print(f'{nu} stopped at epoch: {i} | gpt_loss: {loss_values[0].item()} ,rMAE: {L1_loss}, rRMSE:{L2_loss}(GPT_PINN Stopping Criteria Met)\n')
            break

        loss_values = optimizer.step(closure)
        #GPT_PINN.wi.data = torch.clamp(GPT_PINN.wi.data, min=0.1 ,max=10)
        ...
        if (i % 100 == 0) or (i == epochs_gpt):
            ...
            if (i % 1000 == 0) or (i == epochs_gpt):
                ...
                #print(f"layer1:{GPT_PINN.linears[0].weight.data} and {GPT_PINN.linears[0].bias.data} and layer3:{GPT_PINN.linears[-1].weight.data}")
                if (i % 500 == 0):
                    #Burgers_plot(xt_test,abs(GPT_PINN.forward(xt_test)-Exact_y0),101,201, title=fr"PINN Solution Error $\mu={nu}$")
                    #Burgers_plot(xt_test,GPT_PINN.forward(xt_test),101,201, title=fr"TGPT-PINN Solution $\mu={nu}$")
                    if (i == 1000):
                        lr_gpt = 0.1*lr_gpt
                        optimizer = torch.optim.Adam(GPT_PINN.parameters(), lr=lr_gpt)
