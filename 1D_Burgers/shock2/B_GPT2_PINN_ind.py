...
class GPT2(nn.Module):
    def __init__(self, nu, layers, P, c_initial, resid_data, f_hat, u_exact, xt_test, IC_data,IC_u, BC_bottom, BC_top,xt_RHL,xt_RHR, xt_RH0,xt_RH0L,Nx_train, Nt_train,dt,t_cut=None):
        super().__init__()
        self.layers = layers
        self.nu = nu
        self.t_cut = t_cut
    ...
    def lossR(self,xt_data,P_Iu):
        ...
        range_ind = torch.logical_and(g[:,:1]-u*(g[:,1:]-self.t_cut) >= -1.0, g[:,:1]-u*(g[:,1:]-self.t_cut) < 1.5)
        u_hat = P_Iu(torch.cat((g[:,:1]-u*(g[:,1:]-self.t_cut),self.t_cut*torch.ones(g.shape[0],1).to(device)),1)).to(device)
    ...
    def lossBC(self):
        ...
        loss_BC1 = self.loss_function(u_BC1,self.nu[1]*torch.ones(self.BC_bottom.shape[0],1).to(device))
        loss_BC2 = self.loss_function(u_BC2, torch.zeros(self.BC_top.shape[0],1).to(device))
    ...
    def loss(self,xt,P_Iu):
        """Total Loss Function"""
        loss_R   = self.lossR(xt,P_Iu)
        loss_R0   = self.lossR0()
        loss_IC  = self.lossIC()
        loss_BC  = self.lossBC()
        loss_RH = self.lossRH()
