...
def initial_u(nu, x, t=0):
    ...
    u = nu[1]*torch.ones(x.shape[0],1)
    condition0=(x<=-1/2).cpu()
    condition1=(x>=1/2).cpu()
    u = torch.where(condition1,nu[0]*torch.tensor(1.0), u).to(torch.float32)
...
def exact_u(nu,xt):
    ...
    condition0 = xt[:,0]<= - 0.5
    u=torch.where(condition0,torch.tensor(0.0), u).to(torch.float32)
    condition2 =  torch.logical_and(xt[:,0]>= - 0.5,(xt[:,0]-nu[1]*xt[:,1])> -0.5)
    u=torch.where(condition2,nu[1]*torch.tensor(1.0), u).to(torch.float32)
    condition1 = ((xt[:,0]-((nu[0]+nu[1])*xt[:,1]+1)/2)>=0)
    u = torch.where(condition1,nu[0]*torch.tensor(1.0), u).to(torch.float32)
    return u
